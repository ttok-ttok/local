document.addEventListener('DOMContentLoaded', () => {
  ...
  if (settings.alarmState === 'on') {
    simulateAlarm(settings.hour, settings.minute, alarmList);
  }
});

// ✅ 여기 기존 addAlarmToUI 지우고 dataset 버전으로 교체
...
function simulateAlarm(hour, minute) {
  ...
  setTimeout(() => {
    const alarm = {
      message: '똑똑! 오늘도 건강한 하루체크 도와드릴게요!',
      checked: false,
    };
...
// localStorage 업데이트 (기존 push 말고 전부 재저장)
function saveAlarms() {
  const alarms = [];
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (profileBtn) {
    profileBtn.addEventListener('click', () => {
      window.location.href = '/mypage/mypage.html'; // ✅ 실제 마이페이지 파일 이름으로 수정
    });
  }
});
